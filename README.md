...
![前台列表页：白顶栏 + 品牌色 Hero、左栏分类树、紧凑列表与类型徽标](docs/bbs-index.png)

## 功能特性

- **帖子类型**：讨论（POST）、问答（QUESTION）、公告（ANNOUNCEMENT，仅管理端可发）。三类帖必须归属分类，与置顶正交
- **问答**：作者与版主可标记「已解决」；可选把某条顶层评论设为最佳答案（Console / UC 评论弹窗；设上时同步已解决，取消最佳答案不取消已解决，取消已解决会清掉最佳答案）。改出问答类型时服务端清掉已解决 / 最佳答案残留
...
- **评论**：接入 Halo 官方评论体系。详情页由「评论组件」插件渲染；锁定帖改只读渲染，顶层评论按发表顺序显示冻号（楼主 1、评论从 2，待审占号，置顶不改号，删除不重排）
- **全文搜索**：接入 Halo 搜索，已发布帖子可被站点全局搜索
- **RSS**：经 plugin-feed 输出全站 `/feed/bbs/posts.xml` 与一级分类 `/feed/bbs/categories/{slug}.xml`（未安装则无 RSS，其余照常）
- **前台**：Flarum 两栏——白顶栏 + 品牌色 Hero、左栏分类树、紧凑列表。圆标（公告 / 置顶 / 未解决问答）+ 线框（已解决 / 锁）；详情页 hero 与列表同口径。默认按最后活跃排序
...
- 评论管理复用 Halo 后台「评论」页
- 锁定帖：模板不渲染 `<halo:comment>`，历史评论由前台走 `GET /posts/{name}/comments` 只读渲染（含冻号）。写入被服务端拦截
- 楼号在评论创建时写入（官方编辑器直发由调和器补号），未锁定时官方组件不展示楼号
...
审核通过 / 驳回、帖子新评论走 Halo 官方通知中心：作者在 UC 通知列表收到站内消息（可按偏好开邮件）。作者审核自己的帖子、自己评论自己的帖子不发。事件类型「我的帖子通过审核 / 未通过审核 / 我的帖子收到新评论」会出现在用户通知偏好里。楼中楼回复沿用官方「有人回复了我」（帖子链接为绝对地址）。
